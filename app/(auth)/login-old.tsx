      ...
      <ScrollView contentContainerClassName="flex-grow">
        <View className="flex-1 items-center justify-center min-h-screen px-6 py-8">
          ...
          <View className="flex-row items-end mb-8">
            <Text className="font-playfair text-2xl font-semibold text-primary" style={{ lineHeight: 44 }}>
              my
            </Text>
            <Text className="font-playfair text-6xl font-bold text-primary" style={{ lineHeight: 64 }}>
              Qode
            </Text>
          </View>

          {/* Card Container */}
          <View className={tw.card + " w-full max-w-md"}>
            {/* Header */}
            <View className="mb-6">
              <View className="flex-row items-center justify-between mb-4">
                {currentStep !== 'username' ? (
                  ...
                  >
                    <Feather name="arrow-left" size={20} color="hsl(163.1, 100%, 6.3%)" />
                  </TouchableOpacity>
                ) : <View className="w-10" />}
              ...
              <Text className={tw.h3 + " text-center mb-2"}>
                {getStepTitle()}
              </Text>
              {currentStep === 'otp-verification' && (
                <Text className={tw.bodyMuted + " text-center"}>
            ...
            {/* Error Message */}
            {error ? (
              <View className="bg-destructive/10 border border-destructive rounded-lg px-4 py-3 mb-4">
            ...
            {currentStep === 'username' && (
              <View className="gap-4">
                <View>
                  <Text className={tw.inputLabel}>Email or Account ID</Text>
                  <TextInput
                    className={tw.input}
                    autoCapitalize="none"
                    autoCorrect={false}
                ...
                {!requirePasswordSetup ? (
                  <TouchableOpacity
                    className={tw.buttonPrimary + ` ${isLoading || !username.trim() ? "opacity-50" : ""}`}
                    onPress={checkPasswordStatus}
                    disabled={isLoading || !username.trim()}
                  >
                    {isLoading ? (
                      <ActivityIndicator color="hsl(49.4, 73.9%, 95.5%)" />
                    ) : (
                      <Text className={tw.buttonTextPrimary}>Continue</Text>
                    )}
                  </TouchableOpacity>
                ) : (
                  <>
                    ...
                    <TouchableOpacity
                      className={tw.buttonPrimary + ` ${isLoading ? "opacity-50" : ""}`}
                    ...
                    >
                      {isLoading ? (
                        ...
                        <Text className={tw.buttonTextPrimary}>
                ...
                {isDevelopment && !requirePasswordSetup && (
                  <TouchableOpacity
                    className={`flex-row items-center justify-center border-2 border-accent rounded-lg py-3 px-4 bg-accent/10 ${isLoading ? "opacity-50" : ""}`}
                    onPress={() => {
                    ...
                    disabled={isLoading}
                  >
                    <Feather name="zap" size={20} color="hsl(49.3, 68.6%, 53.7%)" />
                    <Text className="ml-2 text-accent font-lato font-semibold">Dev Mode: Bypass Login</Text>
                  </TouchableOpacity>
                )}
              </View>
            )}
