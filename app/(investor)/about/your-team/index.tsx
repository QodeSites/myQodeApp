...
function Banner() {
  return (
    <View className="mb-6 rounded-sm bg-primary px-4 py-2 text-center text-sm font-semibold text-white flex items-center justify-center gap-2">
...
function Section({
  ...
  return (
    <View className="mb-4" accessible accessibilityLabel={title}>
      <View className="flex flex-row items-center gap-2 mb-2 border-b border-border pb-1">
        {icon}
        <Text className="text-base font-bold text-foreground">{title}</Text>
      </View>
      <View className="text-sm leading-relaxed text-card-foreground">{children}</View>
    </View>
  );
}
  ...
  return (
    <Container className="p-4 w-full rounded-lg bg-card">
      <View className="flex gap-2">
        ...
        <Text className="mb-2 text-md text-muted-foreground">
        Reach the right person quickly, and choose the channel that suits your query.
          {selectedClientCode && (
            <Text className="ml-2 text-primary text-md font-medium">
      ...
      <View className="grid grid-cols-1 gap-2 md:grid-cols-2">
        ...
        {/* Fund Manager */}
        <Section title="Fund Manager" icon={<TrendingUp size={24} className="h-5 w-5 text-primary" />}>
          <Text className="mb-2">
            <Text className="font-semibold">Role:</Text> Oversees your portfolio strategy and ensures alignment with Qode's philosophy.
          </Text>
          <Text>
            <Text className="font-semibold">When to Contact:</Text> Strategy‑specific queries, high‑level portfolio discussions, or during quarterly/annual reviews.
          </Text>
          <TouchableOpacity
            className=" flex flex-row w-full gap-2 rounded-md border border-border bg-primary text-white p-2 items-center justify-center font-semibold"
            onPress={() => setIsStrategyModalOpen(true)}
          >
            <Text className="text-white">Ask a Question on Strategy</Text>
            <Text className="sr-only"> — Opens a short form</Text>
          </TouchableOpacity>
        </Section>

        {/* Investor Relations */}
        <Section title="Investor Relations" icon={<User size={24} className="h-5 w-5 text-primary" />}>
          <Text className="mb-2">
            <Text className="font-semibold">Role:</Text> Your regular point of contact. Shares monthly updates, schedules review calls, and addresses queries. Also helps with operations: onboarding, top‑ups, withdrawals, portal access.
          </Text>
          <Text className="mb-0">
            <Text className="font-semibold">When to Contact:</Text> For reports, account queries, or operational clarifications.
          </Text>
          <View className="flex sm:flex-row flex-col gap-2 items-center">
           <TouchableOpacity
            className=" flex flex-row w-full gap-2 rounded-md border border-border bg-primary text-white p-2 items-center justify-center font-semibold"
              onPress={() => {
            ...
            >
              ...
              <Text className="text-white">Contact IR Team</Text>
            </TouchableOpacity>
            <TouchableOpacity
            className=" flex flex-row w-full gap-2 rounded-md border border-border bg-primary text-white p-2 items-center justify-center font-semibold"
            ...
            >
              ...
              <Text className="text-white">Raise Any Query</Text>
            </TouchableOpacity>
          </View>
        ...
        {/* Book a Call */}
        <Section title="Book A Call" icon={<Calendar size={24} className="h-5 w-5 text-primary" />}>
          <Text className="mb-2">
            <Text className="font-semibold">Purpose:</Text> Quick, hassle‑free scheduling of calls with your IR team.
          </Text>
          <TouchableOpacity
            className=" flex flex-row w-full gap-2 rounded-md border border-border bg-primary text-white p-2 items-center justify-center font-semibold"
          ...
          >
            ...
            <Text className="text-white font-sans">Book a Call</Text>
          </TouchableOpacity>
        </Section>

        {/* WhatsApp / Email */}
        <Section title="WhatsApp/Email" icon={<MessageCircle size={24} className="h-5 w-5 text-primary" />}>
          <View className="mb-2 flex flex-col gap-1">
            <View className="flex flex-row items-center gap-[2px] md:gap-2">
              <Phone className="h-4 w-4 text-primary" />
              <Text className="sm:text-sm text-xs font-semibold">WhatsApp (IR Desk): </Text>
              <Text className="underline decoration-dotted" selectable>
                +91 98203 00028
              </Text>
              <Text className="sm:text-sm text-xs">(9 AM – 5 PM)</Text>
            </View>
          ...
          <Text className="mb-2">
            <Text className="font-semibold">Purpose:</Text> Instant, informal, and quick communication.
          </Text>
        </Section>
      </View>
      ...
      >
        <View className="flex gap-2">
          ...
          {submitStatus === 'success' && (
            <View className="p-3 bg-green-100 border border-green-400 text-green-700 rounded text-sm">
              <Text>
          ...
          {submitStatus === 'error' && (
            <View className="p-3 bg-red-100 border border-red-400 text-red-700 rounded text-sm">
              <Text>
                Failed to send your question. Please try again or contact us directly.
              </Text>
            </View>
          )}
          <View className="flex flex-row mt-1 gap-3 justify-end">
            <TouchableOpacity
              onPress={() => {
                setIsStrategyModalOpen(false);
                setStrategyQuestion('');
            ...
            >
              <Text>{isSubmitting ? 'Sending...' : 'Submit Question'}</Text>
            </TouchableOpacity>
          </View>
        </View>
      </ModalComponent>
      ...
      >
        <View className="flex gap-2">
          ...
          <View className="flex flex-row mt-1 gap-3 justify-end">
            ...
            >
              <Text>{isSubmitting ? 'Sending...' : 'Submit Topic'}</Text>
            </TouchableOpacity>
          </View>
        </View>
      </ModalComponent>
